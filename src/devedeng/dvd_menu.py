...
class dvd_menu(devedeng.interface_manager.interface_manager):
    def __init__(self):
        ...
        self.default_background = os.path.join(self.config.pic_path,
        ...
        self.add_colorbutton("unselected_color", (1, 1, 1, 1),
                             self.update_preview)
        self.add_colorbutton("shadow_color", (0, 0, 0, 0), self.update_preview)
        self.add_colorbutton("selected_color", (0, 1, 1, 1),
                             self.update_preview)
        self.add_colorbutton("background_color", (0, 0, 0, 0.75),
                             self.update_preview)

        self.add_text("title_text", None, self.update_preview)

        self.add_group("position_horizontal", ["left", "center", "right"],
                       "center", self.update_preview)
        self.add_group("at_startup",
                       ["menu_show_at_startup", "play_first_title_at_startup"],
                       "menu_show_at_startup")
        ...
        self.add_filebutton("background_picture", self.default_background,
                            self.update_preview)
        self.add_filebutton("background_music", self.default_sound,
                            self.update_music)
    ...
    def get_estimated_size(self):
        ...
        estimated_size = (
            (self.video_rate + self.audio_rate) * self.sound_length) / 8
    ...
    def update_music(self, b=None):
        ...
        if (video != 0):
            devedeng.message.message_window(
                _("The selected file is a video, not an audio file"),
                _("Error"))
    ...
    def show_configuration(self, file_list):
        ...
        self.wframe_preview_controls = self.builder.get_object(
            "frame_preview_controls")
        ...
        self.wbackground_picture = self.builder.get_object(
            "background_picture")
    ...
    def write_text(self, text, text_type, xl, xr, y, alignment):
        ...
        if (bgcolor is not None):
            self.cr.move_to(x + extents[0] + 2,
                            y - (extents[3] / 2.0) - extents[1] + 2)
            self.cr.set_source_rgba(bgcolor[0], bgcolor[1], bgcolor[2],
                                    bgcolor[3])
    ...
    def paint_menu(self, paint_background, paint_selected, paint_activated,
        ...
        if paint_background:
            ...
            surface = Gdk.cairo_surface_create_from_pixbuf(extra_pixbuf, 1,
                                                           None)
...
#             self.cr.paint()
        ...
        for entry in self.title_list[page_number * entries_per_page:(
                page_number + 1) * entries_per_page]:
    ...
    def paint_base(self, xl, xr, y, type_b):
        ...
        self.cr.curve_to(xr + radius, y, xr + radius, y + height, xr,
                         y + height)
    ...
    def on_drawingarea_preview_draw(self, widget, cr):
        ...
        if (self.sf is None):
            self.paint_menu(True, self.wshow_as_selected.get_active(), False,
                            self.current_shown_page)
            if (self.current_shown_page >= self.pages):
    ...
    def create_menu_stream(self, path, n_page, coordinates):
        ...
        xml_file.write(' image="' + os.path.join(path, "menu_" + str(n_page) +
                                                 '_unselected_bg.png"'))
        xml_file.write(' highlight="' + os.path.join(path, "menu_" + str(
            n_page) + '_selected_bg.png"'))
        xml_file.write(' select="' + os.path.join(path, "menu_" + str(n_page) +
                                                  '_active_bg.png"'))
        xml_file.write(' >\n')
        n_elements = 0
        has_next = False
        ...
        for element in coordinates:
            ...
            if (element[4] == "left"):
                ...
                entry_data["right"] = "boton" + str(n_page) + "x" + str(
                    counter)
            else:
                entry_data["chapters"].append("boton" + str(n_page) + "x" +
                                              str(counter))

            xml_file.write('\t\t\t<button name="boton' + str(n_page) + "x" +
                           str(counter))
            xml_file.write('" x0="' + str(xl) + '" y0="' + str(yt) + '" x1="' +
                           str(xr) + '" y1="' + str(yb) + '"')
            if counter > 0:
            ...
            if (counter < (n_elements - 1)) or ((counter < n_elements) and
                                                (has_next or has_previous)):
                xml_file.write(' down="boton' + str(n_page) + "x")
                xml_file.write(str(counter + 1))
                xml_file.write('"')
            if (element[4] == "left") and (has_next):
                xml_file.write(' right="boton' + str(n_page) + 'x' + str(
                    counter + 1) + '"')
            if (element[4] == "right") and (has_previous):
                xml_file.write(' left="boton' + str(n_page) + 'x' + str(
                    counter - 1) + '"')
    ...
    def create_dvd_menus(self, file_list, base_path):
        ...
        while n_page < self.pages:
            ...
            self.sf.write_to_png(os.path.join(menu_folder, "menu_" + str(
                n_page) + "_bg.png"))
            self.sf = None
            self.paint_menu(False, False, False, n_page)
            self.sf.write_to_png(os.path.join(menu_folder, "menu_" + str(
                n_page) + "_unselected_bg.png"))
            self.sf = None
            ...
            self.sf.write_to_png(os.path.join(menu_folder, "menu_" + str(
                n_page) + "_active_bg.png"))
            entry_data = self.create_menu_stream(menu_folder, n_page,
                                                 coordinates)
            converter = menu_converter()
            final_path = converter.create_menu_mpeg(
                n_page, self.background_music, self.sound_length,
                self.config.PAL, self.video_rate, self.audio_rate, menu_folder,
                self.audio_mp2)
