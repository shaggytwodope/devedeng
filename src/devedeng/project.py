...
class devede_project:
    def __init__(self):
        ...
        self.wmain_window.drag_dest_set(Gtk.DestDefaults.ALL, [],
                                        Gdk.DragAction.COPY)
    ...
    def add_several_files(self, file_list):
        ...
        for efile in file_list:
            ...
            if (new_file.error):
                ...
                self.wliststore_files.append(
                    [new_file, new_file.title_name, True,
                     self.duration_to_string(new_file.get_duration())])
        if (len(error_list) != 0):
            devedeng.message.message_window(
                _("The following files could not be added:"),
                _("Error while adding files"), error_list)
        self.set_interface_status(None)
        self.refresh_disc_usage()
    ...
    def on_delete_file_clicked(self, b):
        ...
        if (ask_w.run(
                _("The file <b>%(X)s</b> <i>(%(Y)s)</i> will be removed.") %
            {"X": element.title_name,
             "Y": element.file_name}, _("Delete file"))):
            element.delete_file()
    ...
    def on_down_file_clicked(self, b):
        ...
        (element, position, model, treeiter) = self.get_current_file()
        if (element is None) or (position ==
                                 (len(self.wfiles.get_model()) - 1)):
            return
    ...
    def refresh_disc_usage(self):
        ...
        self.wdisc_fill_level.set_text(str(addv + int((used / disc_size) *
                                                      100)) + "%")
    ...
    def on_create_disc_clicked(self, b):
        ...
        if (max_files != -1) and (t > max_files):
            devedeng.message.message_window(
                _("The limit for this format is %(l)d files, but your project has %(h)d.")
                % {"l": max_files,
                   "h": t}, _("Too many files in the project"))
            return
        ...
        if os.path.exists(data.path):
            ask_w = devedeng.ask.ask_window()
            retval = ask_w.run(
                _("The selected folder already exists. To create the project, Devede must delete it.\nIf you continue, the folder\n\n <b>%s</b>\n\n and all its contents <b>will be deleted</b>. Continue?")
                % data.path, _("Delete folder"))
            if retval:
                ...
                if self.config.disc_type == "dvd":
                    try:
                        ...
                        pass
                if (self.config.disc_type == "vcd") or (
                        self.config.disc_type == "svcd") or (
                            self.config.disc_type == "cvd"):
                    try:
        ...
        if (self.disc_type == "dvd") and (self.wcreate_menu.get_active()):
            processes, menu_entries = self.menu.create_dvd_menus(file_movies,
                                                                 data.path)
            for p in processes:
        ...
        for movie in file_movies:
            p = movie.do_conversion(os.path.join(movie_folder, "movie_" + str(
                counter) + ".mpg"))
            run_window.add_process(p)
            final_dependencies.append(p)
            counter += 1
        ...
        if (self.disc_type == "dvd"):
            ...
            dvdauthor.create_dvd_project(data.path, data.name, file_movies,
                                         menu_entries, start_with_menu,
                                         self.menu.play_all_c)
    ...
    def on_new_activate(self, b):
        ...
        if w.run(
                _("Close current project and start a fresh one?"),
                _("New project")):
    ...
    def load_project(self, project_file):
        ...
        if "files" in project:
            ...
            if (len(error_list) != 0):
                devedeng.message.message_window(
                    _("The following files in the project could not be added again:"),
                    _("Error while adding files"), error_list)
        self.set_interface_status(None)
        self.refresh_disc_usage()
