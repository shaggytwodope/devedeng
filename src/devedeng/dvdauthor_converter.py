...
class dvdauthor_converter(devedeng.executor.executor):
    ...
    def create_dvd_project(self, path, name, file_movies, menu_entries,
                           start_with_menu, play_all_opt):
        ...
        xml_file = self.create_dvdauthor_xml(path, file_movies, menu_entries,
                                             start_with_menu, play_all_opt)
    ...
    def create_dvdauthor_xml(self, movie_folder, file_movies, menu_entries,
                             start_with_menu, play_all_opt):
        ...
        if onlyone:
            ...
            if play_all_opt and (menu_entries is not None) and (
                    start_with_menu):
                xml_file.write('1;\n')  #auto play all
            else:
                xml_file.write('0;\n')  #do not play all
            ...
            for element in file_movies:
                ...
                xml_file.write('\t\t\t\t\t\tjump titleset ' + str(1 + counter)
                               + ' menu;\n')
                xml_file.write('\t\t\t\t\t}\n')
                counter += 1
            ...
            if self.config.PAL:
                xml_file.write(self.expand_xml(str(os.path.join(
                    self.config.other_path, "base_pal.mpg"))))
            else:
                xml_file.write(self.expand_xml(str(os.path.join(
                    self.config.other_path, "base_ntsc.mpg"))))
                ...
                for menu_page in menu_entries:
                    ...
                    if first_entry:  # here we add some code to jump to each menu
                        for menu2 in range(nmenues - 1):
                            xml_file.write('\t\t\t\t\tif (g1 eq ' + str(
                                menu2 + 1) + ') {\n')
                            xml_file.write('\t\t\t\t\t\tjump menu ' + str(
                                menu2 + 2) + ';\n')
                            xml_file.write('\t\t\t\t\t}\n')
                    ...
                    for nbutton in menu_page["chapters"]:
                        ...
                        if play_all_opt and button_counter == 0:
                            ...
                            xml_file.write('\t\t\t\t\tg0=' + str(title_list[
                                title_counter]) + ';\n')
                            xml_file.write('\t\t\t\t\tjump vmgm menu;\n')
                            title_counter += 1
                        button_counter += 1
                    ...
                    if (menu_page["left"] is not None):
                        xml_file.write('\t\t\t\t<button name="' + menu_page[
                            "left"] + '"> g1=')
                    ...
                    xml_file.write('\t\t\t\t\tjump menu ' + str(menu_number +
                                                                1) + ';\n')
                    xml_file.write('\t\t\t\t</post>\n')
                    xml_file.write('\t\t\t</pgc>\n')
                    menu_number += 1
        ...
        for element in file_movies:
            ...
            if not onlyone:
                xml_file.write("\t\t<menus>\n")
                xml_file.write('\t\t\t<video format="' + pal_ntsc +
                               '" aspect="' + faspect + '"')
                if fwide:
                ...
                xml_file.write('\t\t\t\t\t}\n')
                xml_file.write('\t\t\t\t\tg0=100;\n')
                xml_file.write('\t\t\t\t\tg1=' + str(int(
                    titles / elements_per_menu)) + ';\n')
                ...
                if ispal:
                    xml_file.write(self.expand_xml(str(os.path.join(
                        self.config.other_path, "base_pal"))))
                else:
                    xml_file.write(self.expand_xml(str(os.path.join(
                        self.config.other_path, "base_ntsc"))))
                if fwide:
            ...
            xml_file.write("\t\t<titles>\n")
            xml_file.write('\t\t\t<video format="' + pal_ntsc + '" aspect="' +
                           faspect + '"')
            if fwide:
                xml_file.write(' widescreen="nopanscan"')
            xml_file.write('> </video>\n')
            ...
            xml_file.write('\t\t\t\t<vob file="' + self.expand_xml(
                element.converted_filename) + '" ')
                ...
                if (element.divide_in_chapters):  # add chapters
                    ...
                    while seconds < (element.original_length - 4):
                        ...
                        for idx, m_chapter in enumerate(manual_chapter_list):
                            ...
                            m_chapter = int(m_chapter[0]) * 60 + int(
                                m_chapter[1])  # Change from mm:ss to seconds
                        ...
                        seconds += (toadd * 60)
                xml_file.write(',' + self.return_time((element.original_length
                                                       - 2), False))
            xml_file.write('" />\n')
