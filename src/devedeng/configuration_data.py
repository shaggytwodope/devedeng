        ...
        try:
            ...
            for linea in config_data:
                ...
                if linea[:20] == "subtitle_fill_color:":
                    ...
                    self.subt_fill_color = (float(c[0]), float(c[1]),
                                            float(c[2]), 1.0)
                if linea[:23] == "subtitle_outline_color:":
                    c = linea[23:].strip().split(",")
                    self.subt_outline_color = (float(c[0]), float(c[1]),
                                               float(c[2]), 1.0)
    ...
    def save_config(self):
        ...
        try:
            ...
            if (self.sub_language is not None):
                config_data.write("sub_language:" + str(self.sub_language) +
                                  "\n")
            config_data.write("subtitle_font_size:" + str(
                self.subtitles_font_size) + "\n")
            config_data.write("subtitle_fill_color:" + str(
                self.subt_fill_color[0]) + "," + str(self.subt_fill_color[1]) +
                              "," + str(self.subt_fill_color[2]) + "\n")
            config_data.write("subtitle_outline_color:" + str(
                self.subt_outline_color[0]) + "," + str(
                    self.subt_outline_color[1]) + "," + str(
                        self.subt_outline_color[2]) + "\n")
            config_data.write("subtitle_outilne_thickness:" + str(
                self.subt_outline_thickness))
