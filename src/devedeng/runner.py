...
class runner(GObject.GObject):
    ...
    def on_cancel_clicked(self, b):
        ...
        retval = ask_w.run(
            _("Cancel the current job?"), _("Cancel the current job?"))
        if retval:
            self.error = True
    ...
    def run(self, clear_log=True):
        ...
        for element in self.proc_list:
            ...
            if (element.dependencies is None) and (
                    element.progress_bar is None):
                ...
                if (len(self.progress_bars) > 1):
                    ...
                    self.progress_bars = []
                    break
        self.wtotal.set_text(str(self.total_processes - len(self.proc_list)) +
                             "/" + str(self.total_processes))
        self.wtotal.set_fraction(
            (float(self.total_processes - len(self.proc_list))) /
            (float(self.total_processes)))
