...
class file_movie(devedeng.interface_manager.interface_manager):
    ...
    def __init__(self, file_name, list_files=None):
        ...
        self.add_group("final_size_pal", [
            "size_auto", "size_1920x1080", "size_1280x720", "size_720x576",
            "size_704x576", "size_480x576", "size_352x576", "size_352x288"
        ], "size_auto")
        self.add_group("final_size_ntsc", [
            "size_auto_ntsc", "size_1920x1080_ntsc", "size_1280x720_ntsc",
            "size_720x480_ntsc", "size_704x480_ntsc", "size_480x480_ntsc",
            "size_352x480_ntsc", "size_352x240_ntsc"
        ], "size_auto_ntsc")
        self.add_group("aspect_ratio",
                       ["aspect_auto", "aspect_classic", "aspect_wide"],
                       "aspect_auto")
        self.add_group("scaling",
                       ["add_black_bars", "scale_picture", "cut_picture"],
                       "add_black_bars")
        self.add_group(
            "rotation",
        ...
        self.add_group(
            "deinterlace",
            ["deinterlace_none", "deinterlace_ffmpeg", "deinterlace_yadif"],
            "deinterlace_none")
        self.add_group("actions", [
            "action_stop", "action_play_first", "action_play_previous",
        ...
        self.add_colorbutton("subt_outline_color",
                             self.config.subt_outline_color)
        self.add_float_adjustment("subt_thickness",
                                  self.config.subt_outline_thickness)
        ...
        self.add_enable_disable(
            "divide_in_chapters",
            ["chapter_size_spinbutton", "chapter_list_entry"], [])
        self.add_enable_disable("video_rate_automatic", [],
                                ["video_spinbutton"])
        self.add_enable_disable("audio_rate_automatic", [],
                                ["audio_spinbutton"])
        self.add_enable_disable("sound5_1", ["copy_sound"], [])

        if (self.disc_type == "dvd"):
            self.add_enable_disable("aspect_wide", [], [
                "size_704x576", "size_480x576", "size_352x576", "size_352x288",
                "size_704x480_ntsc", "size_480x480_ntsc", "size_352x480_ntsc",
                "size_352x240_ntsc"
            ])

        self.add_enable_disable("copy_sound", [], [
            "audio_delay_spinbutton", "audio_rate_automatic",
            "audio_spinbutton", "spinbutton_volume", "scale_volume",
            "reset_volume"
        ])

        common_elements = [
            "gop12", "video_rate_automatic", "video_spinbutton",
            "audio_rate_automatic", "audio_spinbutton", "format_pal",
            "format_ntsc", "spinbutton_volume", "scale_volume", "reset_volume",
            "size_auto", "size_1920x1080", "size_1280x720", "size_720x576",
            "size_704x576", "size_480x576", "size_352x576", "size_352x288",
            "size_auto_ntsc", "size_1920x1080_ntsc", "size_1280x720_ntsc",
            "size_720x480_ntsc", "size_704x480_ntsc", "size_480x480_ntsc",
            "size_352x480_ntsc", "size_352x240_ntsc", "aspect_auto",
            "aspect_classic", "aspect_wide", "mirror_horizontal",
            "mirror_vertical", "add_black_bars", "scale_picture",
            "cut_picture", "rotation_0", "rotation_90", "rotation_180",
            "rotation_270", "two_pass_encoding", "deinterlace_none",
            "deinterlace_ffmpeg", "deinterlace_yadif",
        ...
        self.add_enable_disable("no_reencode_audio_video", [],
        ...
        if list_files is None:
            ...
            if (film_analizer.get_film_data(self.file_name)):
                ...
                if self.original_videorate <= 0:
                    ...
                    self.original_videorate = (
                        (8 * self.original_file_size) / self.original_length
                    ) - (self.original_audiorate * self.audio_streams)
    ...
    def get_estimated_size(self):
        ...
        if self.is_mpeg_ps:
            ...
            estimated_size = (
                (self.video_rate_final +
                 (self.audio_rate_final * self.audio_streams) + sub_rate) *
                self.original_length) / 8
    ...
    def set_final_rates(self):
        ...
        elif (self.disc_type == "vcd") or (self.disc_type == "svcd") or (
                self.disc_type == "cvd"):
    ...
    def set_final_size_aspect(self):
        ...
        if self.format_pal:
            ...
            final_size = self.final_size_ntsc[:
                                              -5]  # remove the "_ntsc" from the string

        # for divx or matroska, if the size and the aspect ratio is automatic, just don't change them
        if ((self.disc_type == "divx") or
            (self.disc_type == "mkv")) and (final_size == "size_auto") and (
        ...
        if (self.disc_type == "vcd") or (self.disc_type == "svcd") or (
                self.disc_type == "cvd"):
        ...
        if (self.rotation == "rotation_90") or (
                self.rotation == "rotation_270"):
    ...
    def properties(self):
        ...
        self.builder.add_from_file(os.path.join(self.config.glade,
                                                "wfile_properties.ui"))
        ...
        self.wframe_multiproperties = self.builder.get_object(
            "frame_multiproperties")
        self.wtreeview_multiproperties = self.builder.get_object(
        ...
        if (self.disc_type == "dvd") or (self.disc_type == "divx") or (
                self.disc_type == "mkv"):
        ...
        self.wsize_1920x1080_ntsc = self.builder.get_object(
            "size_1920x1080_ntsc")
        self.wsize_1280x720_ntsc = self.builder.get_object(
            "size_1280x720_ntsc")
        self.wframe_final_size = self.builder.get_object("frame_final_size")
        self.wframe_aspect_ratio = self.builder.get_object(
            "frame_aspect_ratio")
        self.waspect_classic = self.builder.get_object("aspect_classic")
        self.waspect_wide = self.builder.get_object("aspect_wide")
        self.wadd_black_bars_pic = self.builder.get_object(
            "add_black_bars_pic")
    ...
    def on_aspect_classic_toggled(self, b):
        ...
        if (final_aspect < self.original_aspect_ratio):
            self.wadd_black_bars_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_classic_blackbars.png"))
            self.wcut_picture_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_classic_cut.png"))
            self.wscale_picture_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_classic_scale.png"))
        else:
            self.wadd_black_bars_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_wide_blackbars.png"))
            self.wcut_picture_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_wide_cut.png"))
            self.wscale_picture_pic.set_from_file(os.path.join(
                self.config.pic_path, "to_wide_scale.png"))
    ...
    def on_add_subtitles_clicked(self, b):
        ...
        if (subt.run()):
            self.wsubtitles_list.append(
                [subt.filename, subt.encoding, subt.language, subt.put_upper])
    ...
    def do_preview(self):
        ...
        p = self.do_conversion(
            os.path.join(self.config.tmp_folder, "movie_preview.mpg"),
            wpreview.lvalue)
    ...
    def preview_done(self, o, retval):
        ...
        if (retval == 0):
            ...
            disc_player.play_film(os.path.join(self.config.tmp_folder,
                                               "movie_preview.mpg"))
