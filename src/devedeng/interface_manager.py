...
class interface_manager(GObject.GObject):
    ...
    def add_integer_adjustment(self,
                               adjustment_name,
                               default_value,
                               callback=None):
        """ Adds an internal variable with the name text_name, linked to an
    ...
    def update_ui(self, builder):
        ...
        for element in self.interface_colorbuttons:
            ...
            obj = builder.get_object(element[0])
            objcolor = Gdk.Color(
                int(value[0] * 65535.0), int(value[1] * 65535.0),
                int(value[2] * 65535.0))
    ...
    def store_ui(self, builder):
        ...
        for element in self.interface_colorbuttons:
            ...
            exec(
                'self.' + element[0] +
                ' = ((float(objcolor.red))/65535.0, (float(objcolor.green))/65535.0, (float(objcolor.blue))/65535.0, (float(alpha))/65535.0)')
