...
class ffprobe(devedeng.executor.executor):
    ...
    def check_is_installed():
        try:
            handle = subprocess.Popen(["ffprobe", "-h"],
    ...
    def get_film_data(self, file_name):
        ...
        command_line = ["ffprobe", file_name, "-of", "json", "-show_streams",
                        "-loglevel", "quiet"]
    ...
    def process_json(self, file_name, stdout2):
            ...
            if (element["codec_type"] == "video"):
                ...
                if (self.video_streams == 1):
                    ...
                    if ("bit_rate" in element):
                        self.original_videorate = int(float(element[
                            "bit_rate"])) / 1000
                    self.original_fps = self.get_division(element[
                        "avg_frame_rate"])
                    if ("display_aspect_ratio" in element):
                        self.original_aspect_ratio = self.get_division(element[
                ...
                if (self.audio_streams == 1):
                    if ("bit_rate" in element):
                        self.original_audiorate = int(float(element[
                            "bit_rate"])) / 1000
                    self.original_audiorate_uncompressed = int(float(element[
                        "sample_rate"]))

        self.original_size = str(self.original_width) + "x" + str(
            self.original_height)
        if (self.original_aspect_ratio is None) or (
                self.original_aspect_ratio <= 1.0):
            if (self.original_height != 0):
                self.original_aspect_ratio = (float(self.original_width)) / (
        ...
        if self.original_length == -1:  # if it was unable to detect the duration, try to use the human readable format
            ...
            self.config.append_static_log(
                "Using ffprobe human readable format: " + str(stdout2))
